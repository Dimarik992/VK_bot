...
class User(Base):
    ...
    Параметры из таблицы используются для поиска других пользователей ВКонтакте
    """
    __tablename__ = 'user'
...
class UserSearch(Base):
    ...
    search_results = relationship("UserSearchResult", back_populates="user_search")


class Candidate(Base):
    """
    Информация о кандидатах (пользователи ВКонтакте, которые подошли под заданный критерий поиска)
    Если информация об одном кандидате повторяется в разных поисковых запросах, то она перезаписывается
    """
    ...
    search_results = relationship("UserSearchResult", back_populates="candidate")
...
class UserSearchResult(Base):
    ...
    __table_args__ = (UniqueConstraint('user_search_id', 'candidate_vk_id', name='uq_user_search_candidate'),)

    id = Column(Integer, primary_key=True, autoincrement=True)
    user_search_id = Column(Integer, ForeignKey('user_search.id'), nullable=False)
    candidate_vk_id = Column(Integer, ForeignKey('candidate.vk_id'), nullable=False)
...
class UserInterest(Base):
    ...
    __table_args__ = (UniqueConstraint('user_vk_id', 'candidate_vk_id', name='uq_user_candidate_interest'),)

    id = Column(Integer, primary_key=True, autoincrement=True)
    user_vk_id = Column(Integer, ForeignKey('user.vk_id'), nullable=False)
    candidate_vk_id = Column(Integer, ForeignKey('candidate.vk_id'), nullable=False)
    is_viewed = Column(Boolean, default=False)  # флаг, что просмотрен пользователем
    is_liked = Column(Boolean, default=False)   # флаг, что добавлен пользователем в избранное
